...
module Users
  ...
  class InvitationsController < Devise::InvitationsController
    ...
    def find_users_list(user)
      UsersList.find_by(user_id: user.id, list_id: params[:list_id])
    end

    def flash_and_redirect(flash_message)
      flash[:notice] = flash_message
      redirect_to new_users_list_path(list_id: params[:list_id])
    end

    def create_users_list(user)
      UsersList.create!(user_id: user.id, list_id: params[:list_id])
